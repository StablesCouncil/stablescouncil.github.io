...
(function () {
  ...
  var css = ''
    ...
    /* The popup wears the app's agent panel: the lit field (one base tone, two glows), white ink,
       glass for the things that float, Inter. No black surface anywhere (founder 2026-09-06). */
    + '.agent-popup{position:fixed;bottom:104px;right:32px;z-index:9100;width:390px;height:min(680px,calc(100vh - 130px));background-color:#16334f;background-image:radial-gradient(64% 26% at 14% 2%,rgba(103,232,249,0.62),transparent 66%),radial-gradient(72% 30% at 86% 98%,rgba(96,165,250,0.52),transparent 70%);border:1px solid rgba(255,255,255,0.16);border-radius:20px;box-shadow:0 24px 80px rgba(0,0,0,0.5);display:flex;flex-direction:column;overflow:hidden;opacity:0;transform:translateY(16px) scale(0.98);pointer-events:none;transition:opacity 0.25s ease,transform 0.25s ease;font-family:Inter,ui-sans-serif,system-ui,sans-serif;color:#fff}'
    + '.agent-popup.open{opacity:1;transform:translateY(0) scale(1);pointer-events:all}'
    + '.agent-popup-header{display:flex;align-items:center;gap:11px;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.16);flex-shrink:0}'
    + '.agent-popup-logo{width:34px;height:34px;border-radius:8px;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.24);display:flex;align-items:center;justify-content:center;flex-shrink:0}'
    + '.agent-popup-logo img{width:22px;height:22px;object-fit:contain}'
    + '.agent-popup-title{flex:1}'
    + '.agent-popup-title-main{font-size:14px;font-weight:900;color:#fff;font-family:Inter,ui-sans-serif,system-ui,sans-serif;letter-spacing:0.03em}'
    + '.agent-popup-title-sub{font-size:12px;color:rgba(255,255,255,0.58);font-family:Inter,ui-sans-serif,system-ui,sans-serif;margin-top:1px}'
    + '.agent-popup-status{display:flex;align-items:center;gap:6px;font-size:11px;font-weight:700;color:rgba(255,255,255,0.8);font-family:Inter,ui-sans-serif,system-ui,sans-serif;cursor:pointer;padding:6px 12px;border-radius:999px;transition:border-color 0.2s;border:1px solid rgba(255,255,255,0.24);background:rgba(255,255,255,0.12)}'
    + '.agent-popup-status:hover{border-color:#67e8f9}'
    + '.agent-popup-status-dot{width:7px;height:7px;border-radius:50%;background:#22c55e;box-shadow:0 0 0 3px rgba(34,197,94,0.2)}'
    + '.agent-popup-disclaimer{padding:8px 16px;background:transparent;border-bottom:1px solid rgba(255,255,255,0.16);font-family:Inter,ui-sans-serif,system-ui,sans-serif;font-size:11px;font-weight:900;letter-spacing:0.08em;color:#fbbf24;text-align:center;flex-shrink:0;text-transform:uppercase}'
    + '.agent-popup-body{flex:1;position:relative;overflow:hidden}'
    + '.agent-popup-body iframe{width:100%;height:100%;border:none;background:transparent}'
    + '.agent-popup-fallback{display:none;position:absolute;inset:0;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:32px;text-align:center;background:#16334f}'
