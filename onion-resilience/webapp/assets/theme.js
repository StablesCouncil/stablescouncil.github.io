...
(function () {
    ...
    function applyTheme(id, options) {
        ...
        try {
            var bridge = window.StablesCore || window.StablesNative;
            if (bridge && typeof bridge.setLightSystemBars === 'function') {
                bridge.setLightSystemBars(theme === 'paper');
            }
        } catch (_) { /* non-Core surfaces do not own Android system-bar icons */ }
