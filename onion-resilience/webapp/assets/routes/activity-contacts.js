(function () {
  ...
  function persistOwnerId(id) {
    ...
    if (!v) return;
    if (readStoredOwnerId() !== v && typeof window.stablesInvalidateReceiveAddress === 'function') {
      window.stablesInvalidateReceiveAddress();
    }
    _activityOwnerId = v;
  ...
  window.stablesFindIncomingPayment = async function stablesFindIncomingPayment(options) {
  ...
  window.stablesFindMerchantIncomingPayment = function (options) {
    return options && options.invoice ? window.stablesFindMerchantInvoicePayment(options.invoice) : Promise.resolve(null);
  };

  // Dedicated invoice path: ordinary Receive keeps its separate, amount-optional behavior.
  const merchantRejectedCandidates = new Map();
  const merchantVerifiedTransactions = new Map();
  window.stablesFindMerchantInvoicePayment = async function (invoice) {
    if (!DEMO_REAL || !window.StablesRetail || Date.now() > invoice.expiresAt) return null;
    const resp = await mdsCommand('txpow relevant:true max:24');
    if (!mdsOk(resp)) throw new Error('Transaction read unavailable');
    const candidates = txpowsFromResponsePayload(coerceMdsPayloadLocal(resp.response)).slice(0, 24);
    for (const candidate of candidates) {
      const id = normalizeTxHash(candidate.txpowid);
      const key = invoice.id + ':' + id;
      if (!id || merchantRejectedCandidates.has(key)) continue;
      const full = await hydrateTxpowForLiveIngest(candidate);
      if (!full || !txpowHasBody(full)) continue; // Unknown is retryable, never a cached rejection.
      const txn = full.body && (full.body.txn || full.body.transaction);
      if (!txn || !Array.isArray(txn.outputs) || !Array.isArray(txn.state)) continue;
      const match = window.StablesRetail.match(full, invoice, Date.now());
      if (match) {
        merchantVerifiedTransactions.set(match.txpowid, full);
        if (merchantVerifiedTransactions.size > 32) merchantVerifiedTransactions.delete(merchantVerifiedTransactions.keys().next().value);
        return match;
      }
      if (Number(full.header && full.header.timemilli) > Date.now() + 30000) continue;
      merchantRejectedCandidates.set(key, true);
      if (merchantRejectedCandidates.size > 512) merchantRejectedCandidates.delete(merchantRejectedCandidates.keys().next().value);
    }
    return null;
  };
  window.stablesReadMerchantConfirmation = async function (id, invoice) {
    const txid = normalizeTxHash(id);
    if (!/^0x[0-9a-f]{64}$/i.test(txid)) return {known:false};
    const full = merchantVerifiedTransactions.get(txid) || await hydrateTxpowForLiveIngest(txid);
    // Re-establish identity after restart before trusting a locally persisted match.
    if (!full || !invoice || !window.StablesRetail.match(full, Object.assign({}, invoice, {expiresAt:Number.MAX_SAFE_INTEGER}), Date.now())) return {known:false};
    const resp = await mdsCommand('txpow onchain:' + txid);
    if (!mdsOk(resp)) return {known:false};
    const state = coerceMdsPayloadLocal(resp.response) || {};
    if (state.found === false || state.found === 'false') return {known:true, found:false, confirmations:0};
    const confirmations = Number(state.confirmations);
    if (!(state.found === true || state.found === 'true') || !Number.isSafeInteger(confirmations) || confirmations < 0) return {known:false};
    merchantVerifiedTransactions.set(txid, full);
    return {known:true, found:true, confirmations:confirmations};
  };
  ...
  /**
   * Amount that IS already in the displayed balance but has not finished confirming.
   *
   * Founder 2026-09-09: the question on the Wallet was "does the 4,025.93 include the thousand I
   * just claimed". For a claim or a mint the answer is YES, because the optimistic hold credits it
   * the moment it posts (which is why the row carries balanceAlreadyApplied and the overlay must
   * not add it a second time). Those operations therefore need the opposite sentence from an
   * incoming payment: the figure includes this, it is just not final yet.
   */
  function settlingAppliedAmt(x) {
    if (!(x && x.balanceAlreadyApplied === true)) return 0;
    const block = Number(x.block || 0);
    const conf = txConfirmations(x);
    if (block > 0 && conf !== null && conf >= NODE_COIN_CONFIRM_DEPTH) return 0;
  ...
  function getWalletSettlingState() {
    ...
    activitySource().forEach(x => {
      ...
      if (overlay > 0) {
        ...
        const applied = settlingAppliedAmt(x);
        if (applied > 0) appliedByCcy[ccy] = (appliedByCcy[ccy] || 0) + applied;
      }
      flashCount++;
