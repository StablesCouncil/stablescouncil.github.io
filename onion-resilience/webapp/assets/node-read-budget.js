...
(function () {
  ...
  function slot(key) {
    ...
    return budgets[k] || (budgets[k] = { key: k, inFlight: null, lastStartAt: 0, lastOkAt: 0, lastFailedAt: 0, lastReason: null, reads: 0 });
  }
   ...
   * @param opts  { reason, staleMs, minGapMs, skipValue, acceptsResult, failureRetryMs }
  ...
  function request(key, opts, run) {
    ...
    if (isEventReason(reason)) {
      ...
      var failureRetryMs = Number(o.failureRetryMs) || 0;
      if (s.lastFailedAt && failureRetryMs > 0 && (t - s.lastFailedAt) < failureRetryMs) {
        counters.skippedGap++;
        return Promise.resolve(o.skipValue);
      }
    ...
    var tracked = p.then(function (r) {
      ...
      var accepted = true;
      try { if (typeof o.acceptsResult === 'function') accepted = o.acceptsResult(r) === true; }
      catch (_) { accepted = false; }
      s.lastOkAt = accepted ? now() : 0;
      s.lastFailedAt = accepted ? 0 : now();
      return r;
    }, function (err) {
      s.lastOkAt = 0;
      s.lastFailedAt = now();
  ...
  W.stablesNodeReadBudget = {
    ...
    noteRead: function (key) { var s = slot(key); s.lastStartAt = now(); s.lastOkAt = now(); s.lastFailedAt = 0; },
