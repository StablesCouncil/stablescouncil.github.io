...
- **Drive alignment and cleaning:** **`stream_1_app/README.md`** documents the folder map and **`dapp/2-demo/`** as the demo edit target. **`handover_document.md`** adds a **Publication parity** operator checklist (dev → Pages → web → **`2_current`** → brain). **`global_knowledge_base.md`** layer 5 references the **`dapp/`** web mirror. **`dapp/README.md`** links to **`stream_1_app/README.md`**. Showcase **`prod_*` `CHANGELOG`** historical line corrected (Pages path **`stream_1_app/website/dapp/`**).
...
- **`MINIDAPP_VERSIONING.md` (Pages / zip mirror):** **Channel model** section (one shell and UX; stages differ by functionality and backends; showcase as advanced synthetic preview; optional test/prod folders; canonical two-digit labels vs display-only shorthand). **Authoring and ship** now runs through the single active **`website/dapp/`** tree. Handshake **`0_handshake/minidapp_version.md`** points at this path for the next refresh.
