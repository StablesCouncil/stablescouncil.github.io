...
const state = {
  ...
  // Test channel: balances driven by real on-chain (MDS + test token registry for Winiwa/USDw/xWiniwa).
  // No fake simulator balances in 3-test (demo fallbacks cleaned per session summaries).
  balances: {
    minima: 0,
    x: 0,
    m: 0,  // mUSD / USDw in test
    sm: 0,
  ...
  // Factory (prototype UX state) - guarded for test channel (use on-chain/test paths for xWiniwa/USDw mint)
  factory: {
...
// Test channel: delegate faucet to on-chain covenant pour (no demo 10k grant).
// The dedicated faucet page + test-channel-bootstrap handles the real Winiwa pour (1,000 via covenant).
function faucetGrant(amount) {
  const cfg = window.STABLES_CONFIG || {};
  if (cfg.APP_STAGE === 'test' && typeof window.__STABLES_TEST_CLAIM_FAUCET_WINIWA__ === 'function') {
    // Delegate to on-chain test pour
    window.__STABLES_TEST_CLAIM_FAUCET_WINIWA__();
    close('onboardBackdrop');
    navigate('wallet');
    safeRender();
    return;
  }
  // Fallback only for non-test (should not hit in 3-test)
  const granted = Number(amount || 1000);
