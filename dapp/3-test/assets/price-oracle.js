...
const PriceOracle = {
    // Configuration. MEXC ONLY — the same exchange the on-chain rate oracle signs from.
    // CoinGecko is banned in the test channel (one price, chain truth).
    config: {
    ...
    async fetchPrice() {
        try {
            ...
            // No MDS: try MEXC directly from the browser (works when the host/proxy allows it).
            const mexcResponse = await fetch(url, { cache: 'no-store' });
            if (!mexcResponse.ok) {
                throw new Error(`MEXC API error: ${mexcResponse.status}`);
            }
            const mexcData = await mexcResponse.json();
            const price = parseFloat(mexcData && mexcData.price);
            if (!Number.isFinite(price) || price <= 0) {
                throw new Error('MEXC: invalid price');
            }
            this.cache.price = price;
            this.cache.timestamp = Date.now();
            console.log('[Oracle] MINIMA price fetched via MEXC (browser):', price);
            return price;
        } catch (error) {
