...
(function () {
  ...
  /* Which DIRECTIONS are being built right now (transaction not yet posted). This was a single
     boolean, so while any mint or burn was being built every other one was refused, and the founder
     hit it doing exactly what Vault V2 exists for: a mint and a burn at the same time. The lane work
     taught the persisted vault hold about lanes and never touched this flag, which is a second
     definition of "busy" that never learned (D033, standing law 4).
     A mint spends Winiwa, a burn spends xWiniwa, so they take different wallet coins, and V2 sends
     them to different lanes, so they take different vault coins. Nothing is shared and they may
     overlap. Two mints DO compete for the same Winiwa coins, so same-direction still queues. */
  const _mintBurnInFlight = { mint: false, burn: false };
  function _mintBurnAnyInFlight() { return _mintBurnInFlight.mint || _mintBurnInFlight.burn; }
  const TV81_VAULT_HOLD_KEY = 'stables_tv81_vault_hold_v1';
  // VAULT V2 LANES (XWINIWA_VAULT_V2_SAME_BLOCK_DESIGN.md, section 5). When the registry projection
  // carries the V2 vault, the hold is PER LANE: a mint on lane 0 and a burn on lane 1 may be in
  // flight together, so one person can mint and burn in the same block. The build flag above stays
  // global (two builds at once would race on the wallet's own coins). Without V2 the V1 record
  // below is the whole guard, unchanged.
  let _tv81V2Active = false;
  const TV81_VAULT_LANE_HOLD_KEY = 'stables_tv81_vault_lane_hold_v1';
  const _vaultLaneEmptyReads = Object.create(null);
  function tv81VaultLaneHoldsWrite(m) {
    try {
      if (!m || !Object.keys(m).length) localStorage.removeItem(TV81_VAULT_LANE_HOLD_KEY);
      else localStorage.setItem(TV81_VAULT_LANE_HOLD_KEY, JSON.stringify(m));
    } catch (_) { /* ignore */ }
  }
  function tv81VaultLaneHolds() {
    let m = null;
    try { m = JSON.parse(localStorage.getItem(TV81_VAULT_LANE_HOLD_KEY) || 'null'); } catch (_) { m = null; }
    if (!m || typeof m !== 'object') return {};
    const out = {};
    let changed = false;
    Object.keys(m).forEach(function (lane) {
      const r = m[lane];
      if (!r || !r.stateCoinId || !(Number(r.ts) > 0) || Date.now() - Number(r.ts) > TV81_VAULT_HOLD_CEILING_MS) { changed = true; return; }
      out[lane] = r;
    });
    if (changed) tv81VaultLaneHoldsWrite(out);
    return out;
  }
  function tv81VaultLaneHoldSet(lane, stateCoinId, op) {
    const m = tv81VaultLaneHolds();
    m[String(lane)] = { stateCoinId: String(stateCoinId), op: op, ts: Date.now() };
    tv81VaultLaneHoldsWrite(m);
    _vaultLaneEmptyReads[String(lane)] = 0;
  }
  /* Lanes reserved by an operation that is being BUILT but not yet posted. The persisted lane holds
     are only written after a successful txnpost, so for the whole build, which can run a couple of
     minutes, the lane looked free. Two operations started together therefore both read an empty map
     and both chose lane 0, then fought over the same reserve and balance coins. In memory only and
     deliberately: a reservation must not outlive the tab that made it. */
  const _tv81LaneBuilding = Object.create(null);
  function tv81LaneReserve(lane) { _tv81LaneBuilding[String(lane)] = Date.now(); }
  function tv81LaneRelease(lane) { delete _tv81LaneBuilding[String(lane)]; }
  function tv81LaneBusyMap() {
    return Object.assign(Object.create(null), tv81VaultLaneHolds(), _tv81LaneBuilding);
  }
  function tv81VaultLaneHoldClear(lane) {
    const m = tv81VaultLaneHolds();
    delete m[String(lane)];
    tv81VaultLaneHoldsWrite(m);
    delete _vaultLaneEmptyReads[String(lane)];
  }
  ...
  function mintBurnBeginInFlight(op) {
    const dir = (op === 'burn') ? 'burn' : 'mint';
    // Same direction always queues: two mints would both reach for the same Winiwa coins.
    if (_mintBurnInFlight[dir]) return false;
    // Without lanes the vault is one set of coins, so any operation blocks any other.
    if (!_tv81V2Active && _mintBurnAnyInFlight()) return false;
    // V2 lanes: a hold on one lane does not refuse the next operation; the lane picker steps
    // around held lanes and says so in words when every lane is held.
    if (!_tv81V2Active && tv81VaultHoldRecord()) { tv81VaultHoldWatch(); return false; }
  ...
  function mintBurnBusyMessage(op) {
    const dir = (op === 'burn') ? 'burn' : 'mint';
    if (tv81VaultHoldRecord() && !_tv81V2Active) {
      return 'Your previous mint or burn is still being confirmed. This usually takes a few minutes. Wait for it before starting another.';
    }
    /* Name the direction. "A mint or burn is already in progress" was true but unhelpful: it did not
       say that the OTHER direction is free, which under lanes it usually is. */
    return _mintBurnInFlight[dir]
      ? ('Another ' + dir + ' is still being prepared. Wait for it, then start this one.')
  ...
  function mintBurnHoldUntilSettled(stateCoinId, op, lane) {
    if (lane !== undefined && lane !== null) {
      tv81VaultLaneHoldSet(lane, stateCoinId, op);
    } else {
  ...
  function tv81VaultHoldWatch() {
    ...
    const tick = async function () {
      ...
      const laneHolds = tv81VaultLaneHolds();
      const laneIds = Object.keys(laneHolds);
      if (!rec && !laneIds.length) return;
      ...
      // V2 lanes: each held lane is released on its own when its state coin has moved.
      for (let li = 0; li < laneIds.length; li++) {
        const lane = laneIds[li];
        let liveLane = null;
        try { liveLane = await tv81CoinsById(laneHolds[lane].stateCoinId); } catch (_) { liveLane = null; }
        if (Array.isArray(liveLane) && liveLane.length === 0) _vaultLaneEmptyReads[lane] = (_vaultLaneEmptyReads[lane] || 0) + 1; else _vaultLaneEmptyReads[lane] = 0;
        if ((_vaultLaneEmptyReads[lane] || 0) >= 2) {
          tv81VaultLaneHoldClear(lane);
          released = true;
          try { console.log('[tv81-vault] lane ' + lane + ' hold released: its state coin moved (' + String(laneHolds[lane].stateCoinId).slice(0, 18) + ')'); } catch (_) { /* ignore */ }
        }
      }
      if (released) {
        try {
      ...
      if (!tv81VaultHoldRecord() && !Object.keys(tv81VaultLaneHolds()).length) return;
      _vaultHoldWatchTimer = setTimeout(tick, TV81_VAULT_HOLD_POLL_MS);
    };
    _vaultHoldWatchTimer = setTimeout(tick, TV81_VAULT_HOLD_POLL_MS);
  }
  if (tv81VaultHoldRecord() || Object.keys(tv81VaultLaneHolds()).length) tv81VaultHoldWatch(); // a restart mid-confirmation keeps the guard
  window.__STABLES_TEST_MINT_BURN_IN_FLIGHT__ = function () { return _mintBurnAnyInFlight() || (!_tv81V2Active && !!tv81VaultHoldRecord()); };
  /* Exposes the SHIPPED guard so a test can ask it the question a person asks by tapping, without
     posting a real transaction. Reads and clears only this flag; it moves no money. */
  window.__STABLES_TEST_MINT_BURN_GUARD__ = function (action, op) {
    if (action === 'begin') return mintBurnBeginInFlight(op);
    if (action === 'end') { mintBurnEndInFlight(op); return true; }
    if (action === 'why') return mintBurnBusyMessage(op);
    if (action === 'reset') { mintBurnEndInFlight(); return true; }
    if (action === 'v2') return !!_tv81V2Active;
    return null;
  };
  window.__STABLES_TEST_VAULT_HOLD__ = function () { return tv81VaultHoldRecord(); };
  window.__STABLES_TEST_VAULT_LANE_HOLDS__ = function () { return tv81VaultLaneHolds(); };
  /* The lane picker's real view: persisted holds PLUS lanes being built right now. */
  window.__STABLES_TEST_LANE_BUSY__ = function (action, lane) {
    if (action === 'reserve') { tv81LaneReserve(lane); return true; }
    if (action === 'release') { tv81LaneRelease(lane); return true; }
    return Object.keys(tv81LaneBusyMap());
  };
    ...
    if (typeof window.stablesFetchMdsReceiveAddress === 'function') {
      return new Promise(function (resolve, reject) {
        window.stablesFetchMdsReceiveAddress(function (addr) {
          ...
          else reject(new Error('Connect your node and wait for live sync, then try again.'));
        });
      });
    }
  ...
  async function fetchTesterWallet() {
    if (typeof window.stablesFetchMdsReceiveAddress === 'function') {
      return new Promise(function (resolve, reject) {
        window.stablesFetchMdsReceiveAddress(function (display, pair) {
          if (display && pair && pair.hex) resolve({ address: pair.hex, miniaddress: pair.mx || pair.hex });
          else reject(new Error('Connect your node and wait for live sync, then try again.'));
        });
      });
    }
  ...
  // getaddress selects among existing default addresses, so using its miniaddress caused
  ...
  window.stablesForgetWalletFingerprint = function () {
    _walletFingerprintMemo = { value: '', at: 0 };
    if (typeof window.stablesInvalidateReceiveAddress === 'function') window.stablesInvalidateReceiveAddress();
  };
  ...
  function tv81AnchorPublishEnabled() {
    /* THE SAME 'trade' GATE WAS HERE, AND IT WAS THE ONE THAT MATTERED (measured 2026-09-07).
       On 2026-09-07 the order-book gate was found and removed from `tv81AnchorPublishable()` below,
       because this release has no book and the gate made the recovery path unreachable. The identical
       gate one function higher, here, was missed, and this is the one the automatic publisher asks.
       The result looked exactly like a working feature: flipping the switch in Settings calls
       `tv81AnchorPublishSnapshot()` directly, which consults `tv81AnchorPublishable()` (fixed) and
       published one snapshot, while every later automatic pass consulted THIS function (not fixed)
       and silently declined forever. Hence the measurement that opened this: one head at block
       2,301,722, no successor, ageing past its own eight-hour cadence to 10.8 hours while another
       publisher wrote to the same address repeatedly.
       Driving the real app is what found it. Reading the code had produced a confident and wrong
       diagnosis; the runtime reported `tv81AnchorPublishEnabled() === false` with the snapshot 820
       blocks stale and the preference set to on, which no reading of the toggle logic predicted.
       Gate on what a snapshot can actually CARRY, exactly as `tv81AnchorPublishable()` does. */
    if (!tv81AnchorPublishable().length) return false;
    /* ON unless the person turned it off (founder 2026-09-07). The snapshot is how a fresh install
       finds the faucet and the vault with no server anywhere, and one serves everybody: an app only
       writes one when nobody has written one in the last 600 blocks, about eight hours, so across
       the whole channel it costs a handful of dust transactions a day rather than one per person.
       Only an explicit '0' turns it off, so a wallet that never opened Settings still helps. */
    try { return localStorage.getItem(TV81_ANCHOR_PUBLISH_PREF) !== '0'; } catch (_) { return true; }
  }
  let _tv81AnchorPublishing = false;
  /* What a snapshot may carry in THIS release. It is the dependency-free recovery path: a fresh node
     whose unpruned window no longer reaches the faucet and vault coins reads their proofs from the
     chain itself and can then claim and mint. So it is gated on what it actually carries, never on
     the order book alone. Gating it on 'trade' made the path unreachable in the shipped release
     (2026-09-07: no snapshot for this generation existed on chain), which the publisher's own
     "snapshots stay publishable even when the book is empty" note shows was never the intent. */
  function tv81AnchorPublishable() {
    return ['trade', 'faucet', 'xwiniwa-mint-burn'].filter(function (f) { return releaseFeatureAllowed(f); });
  }
  ...
  async function tv81MaintenanceRecords(registry, recipient) {
      ...
      try {
        const vaddrs = [
          String(((registry || {}).xwiniwa_vault || {}).address || '').toLowerCase(),
          String(((tv81VaultV2(registry) || {}).address) || '').toLowerCase()
  ...
  window.tv81SetAnchorPublish = function (on) {
    if (on && !tv81AnchorPublishable().length && !releaseRequireFeature('faucet', 'Publishing on-chain proofs')) {
  ...
  if (tv81Exclusive) {
    try {
      tv81AppRegistry().then(function (r) {
        const v2Pre = tv81VaultV2(r);
        _tv81V2Active = !!v2Pre;
        tv81VaultMiniaddr = String((v2Pre || r.xwiniwa_vault || {}).miniaddress || (v2Pre || r.xwiniwa_vault || {}).address || '');
        ...
        add((v2Pre || {}).address, 'xwiniwa');
        add((v2Pre || {}).miniaddress, 'xwiniwa');
  ...
  // ---- VAULT V2: N lanes at one address (XWINIWA_VAULT_V2_SAME_BLOCK_DESIGN.md, section 5) ----
  // The registry projection carries the V2 vault under `xwiniwa_vault_v2`; a build can be pointed
  // at another entry (the dust-proof, `xwiniwa_vault_v2_dustprove`) for a proof run through
  // TEST_XWINIWA_VAULT_V2_KEY or the local override key. Absent, nothing changes: V1 runs as before.
  function tv81VaultV2Key() {
    let k = '';
    try { k = String(localStorage.getItem('stables_tv81_vault_v2_key') || '').trim(); } catch (_) { k = ''; }
    if (!k) k = String(cfg.TEST_XWINIWA_VAULT_V2_KEY || '').trim();
    return k || 'xwiniwa_vault_v2';
  }
  function tv81VaultV2(registry) {
    const v = (registry || {})[tv81VaultV2Key()];
    if (!v || !v.address || !v.script || !(Number(v.lanes) >= 2)) return null;
    return v;
  }
  function tv81LanePort(vault, which) {
    const p = (vault && vault.lane_ports) || {};
    const d = which === 'reserve' ? 24 : (which === 'balance' ? 90 : 35);
    return Number(p[which] != null ? p[which] : d);
  }
  function tv81LanePoolAtoms(lane) {
    return lane && lane.pool ? tokenDisplayToAtoms8(String(lane.pool.tokenamount || '0')) : 0n;
  }
  // The lanes at a V2 address: { lane, reserve, balance, pool } per lane id seen, sorted by id.
  // Same port checks as the V1 triple, grouped by the lane port each coin class carries.
  async function tv81VaultLanes(vault) {
    const unspent = await tv81VaultUnspentCoins(vault);
    const rp = tv81LanePort(vault, 'reserve');
    const bp = tv81LanePort(vault, 'balance');
    const lanes = Object.create(null);
    const at = function (l) {
      const k = String(l);
      if (!lanes[k]) lanes[k] = { lane: Number(l), reserve: null, balance: null, pool: null };
      return lanes[k];
    };
    const amt = function (c) { return tokenDisplayToAtoms8(String((c && c.tokenamount) || '0')); };
    for (let i = 0; i < unspent.length; i++) {
      const c = unspent[i];
      const t = String(c.tokenid || '').toLowerCase();
      if (t === xwiniwaTokenId.toLowerCase() && String(readStatePort(c, 27)) === '8102') {
        const l = readStatePort(c, rp);
        if (l == null) continue;
        if (!at(l).reserve || amt(c) > amt(at(l).reserve)) at(l).reserve = c;
      } else if (t === '0x00' && String(readStatePort(c, 62)) === '8107') {
        const l = readStatePort(c, bp);
        if (l == null) continue;
        const cur = at(l).balance;
        if (!cur || BigInt(String(readStatePort(c, 88) || '0')) > BigInt(String(readStatePort(cur, 88) || '0'))) at(l).balance = c;
      } else if (t === winiwaTokenId.toLowerCase()) {
        const l = readStatePort(c, rp); // a V2 pool coin carries its lane
        if (l == null) continue;
        if (!at(l).pool || amt(c) > amt(at(l).pool)) at(l).pool = c;
      }
    }
    return Object.keys(lanes).map(function (k) { return lanes[k]; }).sort(function (a, b) { return a.lane - b.lane; });
  }
  function tv81LaneCapacity(lane, op, atoms) {
    if (!lane || !lane.reserve || !lane.balance) return false;
    if (op === 0) return tokenDisplayToAtoms8(String(lane.reserve.tokenamount || '0')) > atoms;
    return tv81LanePoolAtoms(lane) >= atoms && BigInt(String(readStatePort(lane.balance, 68) || '0')) >= atoms;
  }
  // Lowest-id lane not held by this app's own in-flight operation that has the capacity (founder
  // choice 2026-09-06: lowest free lane, deterministic and auditable).
  function tv81PickLane(lanes, op, atoms, held) {
    for (let i = 0; i < lanes.length; i++) {
      if (held && held[String(lanes[i].lane)]) continue;
      if (tv81LaneCapacity(lanes[i], op, atoms)) return lanes[i];
    }
    return null;
  }
  async function tv81VaultUnspentCoins(vault) {
    ...
    return coins.filter(function (c) { return c && !c.spent; });
  }
  ...
  async function tv81ReadVaultState() {
    const registry = await tv81AppRegistry();
    const v2 = tv81VaultV2(registry);
    if (v2) {
      const lanes = await tv81VaultLanes(v2);
      const ready = lanes.filter(function (l) { return l.reserve && l.balance; });
      if (!ready.length) {
        return { deployed: false, status: 'VAULT_COINS_NOT_VISIBLE', reason: 'The vault lanes are not visible yet; wait for chain sync.' };
      }
      let reserve = 0n, pool = 0n, issued = 0n;
      ready.forEach(function (l) {
        reserve += tokenDisplayToAtoms8(String(l.reserve.tokenamount || '0'));
        pool += tv81LanePoolAtoms(l);
        issued += BigInt(String(readStatePort(l.balance, 68) || '0'));
      });
      return {
        deployed: true,
        status: 'DEPLOYED_UNVERIFIED',
        address: v2.address,
        parLaw: v2.par_law || (registry.xwiniwa_vault || {}).par_law,
        lanes: Number(v2.lanes),
        lanesReady: ready.length,
        reserveXwiniwa: tv81AtomsToDisplay8(reserve),
        poolWiniwa: tv81AtomsToDisplay8(pool),
        issuedXwiniwa: tv81AtomsToDisplay8(issued),
        balanceNonce: ready.map(function (l) { return String(readStatePort(l.balance, 88) || '0'); }).join(','),
        navPar: '1.0'
      };
    }
  ...
  async function tv81VaultOnChain(op, amount) {
    ...
    const v1 = registry.xwiniwa_vault;
    const v2 = tv81VaultV2(registry);

    // V2 lanes first: a mint always goes to a lane; a burn goes to a lane whose pool covers it and
    // otherwise drains V1 while V1 still holds a pool (founder choice 2026-09-06: V1 drains). A
    // burn no single lane covers is refused in words with the largest lane's amount; it is never
    // split across lanes on the person's behalf (whole-atom law).
    if (v2) {
      await ensureCovenantTracked(v2.address, v2.script, 'tracking the xWiniwa vault lanes', 30000);
      const lanes = await tv81VaultLanes(v2);
      const ready = lanes.filter(function (l) { return l.reserve && l.balance; });
      if (!ready.length) throw new Error('The vault lanes are not visible yet. Wait for chain sync and retry.');
      const held = tv81LaneBusyMap();
      const lane = tv81PickLane(ready, op, atoms, held);
      if (lane) {
        tv81LaneReserve(lane.lane);
        try {
          return await tv81VaultExecute(v2, lane, lane.lane, op, atoms, amtDisplay, wallet, registry);
        } finally {
          /* Released either way. On success the persisted lane hold written at txnpost takes over
             and keeps the lane busy until the chain settles it; on failure the lane is free again
             at once, because nothing was spent. */
          tv81LaneRelease(lane.lane);
        }
      }
      const free = ready.filter(function (l) { return !held[String(l.lane)]; }); // held includes lanes being built
      if (!free.length) throw new Error('Every vault lane has one of your operations still confirming. Wait a minute and try again.');
      if (op === 0) throw new Error('No vault lane can issue that much xWiniwa in one operation. Try a smaller amount.');
      let largest = 0n;
      free.forEach(function (l) { const p = tv81LanePoolAtoms(l); if (p > largest) largest = p; });
      if (v1 && v1.address && v1.script && v2.v1_drains !== false) {
        await ensureCovenantTracked(v1.address, v1.script, 'tracking the xWiniwa vault covenant', 30000);
        const v1Coins = await tv81VaultCoins(v1);
        const v1Pool = (v1Coins.pool && v1Coins.balance && v1Coins.reserve) ? tokenDisplayToAtoms8(String(v1Coins.pool.tokenamount || '0')) : 0n;
        const v1Issued = v1Coins.balance ? BigInt(String(readStatePort(v1Coins.balance, 68) || '0')) : 0n;
        if (v1Pool >= atoms && v1Issued >= atoms) {
          /* This is the V1 drain path: a burn no single V2 lane can cover falls back to the ONE
             original vault, which really is one set of coins, so a hold there does refuse. */
          if (tv81VaultHoldRecord()) throw new Error(mintBurnBusyMessage(op));
          return tv81VaultExecute(v1, v1Coins, null, op, atoms, amtDisplay, wallet, registry);
        }
      }
      throw new Error('No single vault lane holds enough Winiwa for this burn. The largest lane can cover '
        + tv81AtomsToDisplay8(largest) + ' xWiniwa; burn at most that in one operation.');
    }
    ...
    return tv81VaultExecute(vault, coins, null, op, atoms, amtDisplay, wallet, registry);
  }

  // One par operation against one vault: V1 (the single triple, laneId null) or one V2 lane's
  // triple (laneId a number). V2 adds the lane ports (reserve 24, transition 35, balance 90),
  // keeps state on the pool continuation, derives the operation id with the lane, uses a draft id
  // of its own per lane, and holds that lane only.
  async function tv81VaultExecute(vault, coins, laneId, op, atoms, amtDisplay, wallet, registry) {
    const isLane = laneId !== null && laneId !== undefined;
    ...
    const generationId = String(vault.generation_id || registry.generation_id || '').toLowerCase();
    const idValues = [generationId, String(op === 0 ? 1 : 2), (prev88 + 1n).toString(), recipient, atoms.toString()];
    if (isLane) idValues.push(String(laneId));
    const encoded = idValues.map(function (v) { return String(v).length + ':' + v; }).join('|');
    const operationId = await sha256HexUtf8((isLane ? 'stables|tv81|vault-op|v2|6|' : 'stables|tv81|vault-op|v1|5|') + encoded + '\n');
    ...
    if (isLane) { ports[24] = String(laneId); ports[35] = String(laneId); ports[90] = String(laneId); }

    const stateDust = String(coins.balance.amount || '0.0001');
    /* The draft id used to be keyed by lane alone, so a mint and a burn on the same lane shared
       one id, and the line below ("clear the previous draft") made the second operation DELETE the
       first one's half-built transaction. The founder hit exactly that on 2026-09-08 running a mint
       and a burn together: the burn settled and the mint died with "Transaction not found:
       stables_tv81_vault_op_l0". Adding the direction separates them, and keeps the cleanup working
       for a retry of the same direction on the same lane, which is what it was there for. */
    const txnId = (isLane ? (TV81_VAULT_TXN_ID + '_l' + laneId) : TV81_VAULT_TXN_ID)
      + (op === 0 ? '_mint' : '_burn');
    try { await directOrMdsCmd('txndelete id:' + txnId, 'clearing the previous vault draft', 15000); } catch (_) { /* ignore */ }
    const steps = [
      'txncreate id:' + txnId,
      'txninput id:' + txnId + ' coinid:' + coins.reserve.coinid,
      'txninput id:' + txnId + ' coinid:' + coins.balance.coinid,
    ];
    if (poolIn > 0n) steps.push('txninput id:' + txnId + ' coinid:' + coins.pool.coinid);
    for (let u = 0; u < userCoins.length; u++) {
      steps.push('txninput id:' + txnId + ' coinid:' + userCoins[u].coinid);
    }
    if (op === 0) {
      const reserveLeft = tv81AtomsToDisplay8(reserveAtoms - atoms);
      steps.push('txnoutput id:' + txnId + ' amount:' + amtDisplay + ' address:' + wallet.address + ' tokenid:' + xwiniwaTokenId + ' storestate:false');
      steps.push('txnoutput id:' + txnId + ' amount:' + reserveLeft + ' address:' + vault.address + ' tokenid:' + xwiniwaTokenId + ' storestate:true');
      steps.push('txnoutput id:' + txnId + ' amount:' + tv81AtomsToDisplay8(next63) + ' address:' + vault.address + ' tokenid:' + winiwaTokenId + ' storestate:' + (isLane ? 'true' : 'false'));
      steps.push('txnoutput id:' + txnId + ' amount:' + stateDust + ' address:' + vault.address + ' tokenid:0x00 storestate:true');
      if (changeAtoms > 0n) {
        steps.push('txnoutput id:' + txnId + ' amount:' + tv81AtomsToDisplay8(changeAtoms) + ' address:' + wallet.address + ' tokenid:' + winiwaTokenId + ' storestate:false');
      }
    } else {
      const reservePlus = tv81AtomsToDisplay8(reserveAtoms + atoms);
      steps.push('txnoutput id:' + txnId + ' amount:' + amtDisplay + ' address:' + wallet.address + ' tokenid:' + winiwaTokenId + ' storestate:false');
      steps.push('txnoutput id:' + txnId + ' amount:' + reservePlus + ' address:' + vault.address + ' tokenid:' + xwiniwaTokenId + ' storestate:true');
      if (next63 > 0n) {
        steps.push('txnoutput id:' + txnId + ' amount:' + tv81AtomsToDisplay8(next63) + ' address:' + vault.address + ' tokenid:' + winiwaTokenId + ' storestate:' + (isLane ? 'true' : 'false'));
      }
      steps.push('txnoutput id:' + txnId + ' amount:' + stateDust + ' address:' + vault.address + ' tokenid:0x00 storestate:true');
      if (changeAtoms > 0n) {
        steps.push('txnoutput id:' + txnId + ' amount:' + tv81AtomsToDisplay8(changeAtoms) + ' address:' + wallet.address + ' tokenid:' + xwiniwaTokenId + ' storestate:false');
      }
    }
    ...
    mintBurnHoldUntilSettled(coins.balance.coinid, op, isLane ? laneId : undefined);
    noteUserCoinsInFlight(userCoins);
    ...
    extracted.vaultLane = isLane ? laneId : null;
    extracted.vaultAddress = vault.address;
    try { console.log('[tv81-vault] ' + (op === 0 ? 'mint' : 'burn') + ' posted at ' + (isLane ? 'V2 lane ' + laneId : 'V1') + ' (' + String(vault.address).slice(0, 12) + ')'); } catch (_) { /* ignore */ }
    return extracted;
  }
  ...
  window.tv81DecorateExchangeDropdowns = function () {
    ...
    ['exFromCcyPanel', 'exToCcyPanel'].forEach(function (pid) {
      ...
      panel.querySelectorAll('.ex-ccy-dd__row').forEach(function (row) {
        ...
        /* "Unavailable" rather than "Coming soon" (founder 2026-09-08): it describes the token's
           state in this test rather than promising a date. The same word comes from
           `exDropdownAmountText`, so the decorator and the balance refresh cannot disagree. */
        const amtEl = row.querySelector('.ex-ccy-dd__row-amt');
        if (amtEl && !active) amtEl.textContent = (window.STABLES_UNAVAILABLE_LABEL || 'Unavailable');
      });
  ...
  /* The Exchange rate row states a rate and nothing else, in the same shape as Mint and Burn, and
     tapping it turns the pair around. `__EX_RATE_INVERTED` is the flag `flipRatePill()` toggles, so
     one tap flips every surface the same way. A null rate leaves the placeholder.
     Before this the live path overwrote the pill with sentences ("Reading the order book", "No
     offers on this side yet", "Best N Winiwa per xWiniwa") and, when it did show a rate, wrote it
     directly and ignored the flip, so tapping did nothing on this page. */
  function tv81ExSetRate(winiwaPerXw) {
    const pill = document.getElementById('ratePill');
    if (!pill) return;
    const r = Number(winiwaPerXw);
    if (!Number.isFinite(r) || r <= 0) { pill.textContent = '—'; return; }
    pill.textContent = window.__EX_RATE_INVERTED
      ? ('1 Winiwa = ' + tv81G(1 / r, 4) + ' xWiniwa')
      : ('1 xWiniwa = ' + tv81G(r, 4) + ' Winiwa');
  }
  /* Sentences belong on the status line under the row, which already hides itself when quiet. */
  function tv81ExSay(text, tone) {
  ...
  window.tv81ExSetRate = tv81ExSetRate;

  /* The Exchange action button, in the app's registered roles rather than an inline opacity.
     It used to set `style.opacity = '0.55'` and leave the `btn-primary` class on, so a blocked
     action still rendered as the bright recommended next step and invited a press the code was
     always going to refuse (founder 2026-09-08, from a 555 Winiwa quote against a book covering
     0.44). `btn-disabled` paints the muted surface, which is the same swap `feedback.js` makes. */
  function tv81ExSetAction(enabled, label) {
    ...
    btn.classList.add(enabled ? 'btn-primary' : 'btn-disabled');
    btn.setAttribute('aria-disabled', enabled ? 'false' : 'true');
    if (label) btn.textContent = label;
  }
  window.tv81ExSetAction = tv81ExSetAction;

  window.tv81ExchangeCalc = function () {
    if (!tv81Exclusive) return false;
    /* EXCHANGE IS NOT IN THIS RELEASE, so the page must not behave as though it were (founder
       2026-09-08: "we should not display this message and it should be made clear that this
       function will be made available once the stablecoins are minted").
       `enabledFeatures` in the release profile does not list `exchange` or `trade`, and the public
       scope boundary says trading is switched off. The route stays reachable under D027, which
       wants a deferred operation left in its authentic place, looking unavailable and refusing.
       But `TV81_EX_ACTIVE` was a SECOND list of what is live, and it never consulted the profile,
       so the page ran real order-book quoting and reported the depth of a book this test does not
       really have. That is how "The book only covers 0.4425 Winiwa of this size" reached a tester:
       a true sentence about a market that is not part of the release. One source of truth. */
    if (!releaseFeatureAllowed('exchange')) {
      const exTo0 = document.getElementById('exTo');
      if (exTo0) exTo0.value = '';
      tv81ExSetRate(null);
      tv81ExSay('Exchanging opens when the stablecoins are minted. Until then you can mint and burn xWiniwa on the Mint page.');
      tv81ExSetAction(false, 'Exchange now');
      return true;
    }
    ...
    if (live && bookPair) {
      ...
      if (!book) {
        if (exTo) exTo.value = '';
        tv81ExSetRate(null); tv81ExSay('Reading the order book.');
        tv81ExSetAction(false, 'Exchange now');
      } else if (!(amt > 0)) {
        if (exTo) exTo.value = '';
        const bestRows = side === 'buy' ? (book.orders.asks || []) : (book.orders.bids || []);
        tv81ExSetRate(bestRows.length ? (Number(bestRows[0].priceAtoms) / 1e8) : null);
        tv81ExSay(bestRows.length ? '' : 'No offers on this side yet.');
        tv81ExSetAction(false, 'Exchange now');
      } else if (!walk) {
        if (exTo) exTo.value = '';
        tv81ExSetRate(null); tv81ExSay('No offers on this side yet.');
        tv81ExSetAction(false, 'Exchange now');
      } else {
        ...
        tv81ExSetRate(walk.effective);
        /* A size the book cannot cover is REFUSED by `executeExchangeNow` with a toast, so the
           button must not look like the recommended action. The impact line beside it already
           says how much is covered and what to do instead. An expensive-but-allowed fill stays
           enabled; only a genuine shortfall disables. */
        tv81ExSetAction(!(walk.shortfall > 1e-9), 'Exchange now');
      }
    ...
    if (live) {
      ...
      tv81ExSetRate(1); tv81ExSay('');
      tv81ExSetAction(true, 'Exchange now');
    } else {
      if (exTo) exTo.value = '';
      tv81ExSetRate(null);
      tv81ExSay((from === to) ? 'Pick two different tokens.' : (from + ' to ' + to + ' is not part of this test yet.'));
      tv81ExSetAction(false, 'Coming soon');
    }
    try { window.tv81SyncExchangeAvail(); } catch (_) { /* ignore */ }
    return true;
  };
  ...
  window.stablesRefreshXwiniwaReserveLevel = async function stablesRefreshXwiniwaReserveLevel(attempt) {
    ...
    try {
      ...
      if (tv81Exclusive) {
        try {
          const regPre = await tv81AppRegistry();
          const tvVault = tv81VaultV2(regPre) || regPre.xwiniwa_vault || {};
      ...
      if (tv81Exclusive) {
        const registry = await tv81AppRegistry();
        const v2Disp = tv81VaultV2(registry);
        const vault = v2Disp || registry.xwiniwa_vault || {};
        let provenCoins = { balance: null, reserve: null, pool: null };
        let v1DrainPool = 0;
        if (v2Disp) {
          // V2: the lanes are summed for the surface, proven when at least one lane is complete;
          // V1's remaining pool still counts for "can burn" while V1 drains.
          const lanesDisp = (await tv81VaultLanes(v2Disp)).filter(function (l) { return l.reserve && l.balance; });
          if (lanesDisp.length) {
            let rsv = 0n, pl = 0n, iss = 0n;
            lanesDisp.forEach(function (l) {
              rsv += tokenDisplayToAtoms8(String(l.reserve.tokenamount || '0'));
              pl += tv81LanePoolAtoms(l);
              iss += BigInt(String(readStatePort(l.balance, 68) || '0'));
            });
            provenCoins = {
              balance: { state: [{ port: 68, data: iss.toString() }] },
              reserve: { tokenamount: tv81AtomsToDisplay8(rsv) },
              pool: pl > 0n ? { tokenamount: tv81AtomsToDisplay8(pl) } : null
            };
          }
          try {
            if (registry.xwiniwa_vault && v2Disp.v1_drains !== false) {
              const v1c = await tv81VaultCoins(registry.xwiniwa_vault);
              if (v1c.pool && v1c.balance && v1c.reserve) v1DrainPool = Number(v1c.pool.tokenamount || 0);
            }
          } catch (_) { v1DrainPool = 0; }
        } else {
          provenCoins = await tv81VaultCoins(vault);
        }
        if (provenCoins.balance && provenCoins.reserve) {
          ...
          publishXwiniwaVaultProofState('ready', {
            ...
            canBurn: (!!provenCoins.pool && poolAmount > 0 && issuedAtoms > 0n) || v1DrainPool > 0,
  ...
  // Coins already spent by one of this app's operations that has not mined yet. The node still
  // lists them as sendable until the block lands, so a second operation built right behind the
  // first would pick the same largest note and die as a double spend (seen 2026-09-07: two mints
  // back to back on the web preview, the second never mined). Remembered for the hold ceiling.
  const _userCoinsInFlight = Object.create(null);
  function noteUserCoinsInFlight(coins) {
    const now = Date.now();
    (coins || []).forEach(function (c) { if (c && c.coinid) _userCoinsInFlight[String(c.coinid).toLowerCase()] = now; });
  }
  function userCoinInFlight(c) {
    const key = c && c.coinid ? String(c.coinid).toLowerCase() : '';
    const ts = key ? _userCoinsInFlight[key] : 0;
    if (!ts) return false;
    if (Date.now() - ts > TV81_VAULT_HOLD_CEILING_MS) { delete _userCoinsInFlight[key]; return false; }
    return true;
  }
  ...
  /* One honest sentence for a node that refused an operation on size grounds. It distinguishes the
     two errors, which have different causes, and it never tells someone to turn on a setting they
     have already turned on. */
  function stablesTooManyNotesMessage(raw) {
    const replyCap = /Result too long/i.test(String(raw || ''));
    if (replyCap) {
      return 'The node could not return a reply that large. Your notes fit the transaction, so combining them '
        + 'may not help. Try a smaller amount, or use Tidy up notes in Wallet management first.';
    }
    if (stablesAutoCombineOn()) {
      return 'Too many notes for one transaction, and combining them automatically did not get it under the '
        + 'limit. Use Tidy up notes in Wallet management, or try a smaller amount.';
    }
    return 'Too many notes for one transaction. Turn on "Combine notes when a payment needs it" in Wallet '
      + 'management, or use Tidy up notes, then try again.';
  }
  ...
  function stablesSetOptimisticBalance(ccy, value, dir) {
    ...
    try {
      ...
      const prevAvail = Number(prev.available);
      /* OWNED and SPENDABLE are two different questions (see getVaultOwnedBalance): the balance a
         person reads may include a credit that has landed, while only coins at depth can actually
         be moved. This hold used to write BOTH fields, so an incoming credit raised `available`
         too and MAX offered coins the chain would refuse: the founder claimed 1,000 from the
         faucet, pressed MAX on Mint and got "Not enough Winiwa in your wallet for 6468.92507698"
         against a spendable 5,468.93 (2026-09-09).
         A CREDIT raises what is owned and leaves spendable alone. A DEBIT lowers both, because
         money on its way out is neither owned nor spendable. The node refresh below already
         follows exactly this rule (available = node sendable, total may carry the optimistic
         maximum); this write now agrees with it instead of contradicting it for a few minutes. */
      d[ccy] = Object.assign({}, prev, {
        total: next,
        available: (dir === 'out')
          ? (Number.isFinite(prevAvail) ? Math.min(prevAvail, next) : next)
          : (Number.isFinite(prevAvail) ? prevAvail : 0)
      });
    } catch (_) { /* ignore */ }
  }
  ...
  window.stablesRefreshLiveNodeBalances = function stablesRefreshLiveNodeBalances(options) {
    ...
    return budget.request('wallet-balance', {
      reason: reason,
      acceptsResult: function (result) { return !!result && result.proofReady === true; },
      failureRetryMs: isApk ? 12000 : 4000,
  ...
  async function _executeMintWablesTestConfirmed(mintAmt, winiwaCost, address, winiwaBefore) {
    ...
    if (mintBurnMode === 'covenant' && !mintBurnBeginInFlight('mint')) {
      return showToast(mintBurnBusyMessage('mint'), { tone: 'amber', durationMs: 6000 });
    }
  ...
  async function _executeBurnWablesTestConfirmed(burnAmt, reclaimWiniwa, address, stableBal) {
    ...
    if (mintBurnMode === 'covenant' && !mintBurnBeginInFlight('burn')) {
      return showToast(mintBurnBusyMessage('burn'), { tone: 'amber', durationMs: 6000 });
    }
    ...
    if (mintBurnMode === 'covenant') {
      ...
      try {
        ...
        setTimeout(function () {
          ...
          })().catch(function () { /* ignore */ }).finally(function () { mintBurnEndInFlight('burn'); });
        }, 1500);
        if (typeof window.stablesRefreshPendingSettlement === 'function') {
          setTimeout(function () { try { window.stablesRefreshPendingSettlement(); } catch (_) {} }, 1500);
        }
  ...
  async function _executeMintXwmTestConfirmed(mintAmt, address, winiwaBefore, collWiniwa) {
    const handlerFlowStartMs = Date.now();
    if (xwiniwaMintBurnMode === 'covenant' && !mintBurnBeginInFlight('mint')) {
      return showToast(mintBurnBusyMessage('mint'), { tone: 'amber', durationMs: 6000 });
    }
    ...
    try {
      ...
      if (/Result too long|TxPoW size too large/i.test(errMsg)) {
        /* This used to say "Turn on Combine notes when a payment needs it", which the founder read
           on 2026-09-08 with that setting already ON, on a path that already combines automatically.
           Two separate faults were behind one sentence. The mint and burn paths call
           `gatherUserCoinsFitted`, which checks the fit and combines first, so by the time this
           handler runs the notes DID fit the estimate (14 notes plus 3 covenant inputs against a cap
           of 36) and the node still refused. And "Result too long" is a REPLY-size limit, not a
           transaction-size one, so it does not mean too many notes at all. Never instruct someone to
           enable a setting that is already enabled: say what happened and what is left to try. */
        errMsg = stablesTooManyNotesMessage(errMsg);
      }
    ...
    try {
      ...
      setTimeout(function () {
        ...
        })().catch(function () { /* ignore */ }).finally(function () { mintBurnEndInFlight('mint'); });
      }, 1500);
      if (typeof window.stablesRefreshPendingSettlement === 'function') {
        setTimeout(function () { try { window.stablesRefreshPendingSettlement(); } catch (_) {} }, 1500);
      }
  ...
  async function _executeBurnXwmTestConfirmed(burnAmt, address, xBal, winiwaOut) {
    const handlerFlowStartMs = Date.now();
    if (xwiniwaMintBurnMode === 'covenant' && !mintBurnBeginInFlight('burn')) {
      return showToast(mintBurnBusyMessage('burn'), { tone: 'amber', durationMs: 6000 });
    }
      ...
      try {
        const anchorBootAttempt = function (attempt) {
          tv81AppRegistry().then(function (registry) {
            ...
            setTimeout(function () {
              /* Publish if the shared snapshot is missing or past its cadence. Measured 2026-09-07:
                 the live snapshot for our generation reached 10.8 hours old against its own eight-hour
                 ceiling, one head, no successor, while another publisher wrote to the same address
                 repeatedly. Publishing was ON by default and never happened, for two compounding
                 reasons, both fixed here.

                 1. THE BOOT PASS NEVER PUBLISHED. This check lived only inside the repeat callback
                    below, so opening the app and using it normally contributed nothing, ever.
                 2. THE REPEAT IS VISIBILITY-GATED AND HALF AN HOUR LONG. `stablesRepeatWhileVisible`
                    arms only while the app is on screen and this job passes no `whileHidden`, so the
                    app had to sit foregrounded for a continuous 30 minutes before one attempt. Nobody
                    holds a wallet app open for 30 minutes.

                 So the switch in Settings was true and the behaviour was false, the same shape as the
                 resync banner that was only re-checked on connection-state changes. Fresh installs
                 forward-capture the NEXT snapshot rather than an old one, so "nobody publishes" is not
                 a degraded path, it is a dead one.

                 THE FIX IS THE BOOT CHECK, NOT A FASTER TIMER. See the note on the repeat below.

                 The publisher still refuses honestly on its own terms (no publishable release feature,
                 no funded wallet, already publishing), so calling it more often costs a read and a
                 refusal, not a transaction. It writes only when nobody else has written inside the
                 cadence, which is what keeps one proof serving everybody. */
              /* SAY THE DECISION OUT LOUD, EVERY TIME, INCLUDING WHEN IT IS TO DO NOTHING.
                 A working publisher is silent and a broken one is silent, and that is exactly how the
                 gate above survived a day looking healthy: the only way to tell the two apart was to
                 watch the chain from somewhere else and notice nothing arriving. One line per pass
                 fixes that. It names the verdict, the reason, and the numbers the verdict was made
              ...
              const anchorPublishIfStale = function (why) {
                const say = function (verdict, reason, s) {
                  try {
                    console.log('[STABLES-ANCHOR] publish ' + verdict + ' (' + reason + ') pass=' + why
                      + ' state=' + ((s && s.state) || 'unknown') + ' headAge=' + ((s && s.headAge) != null ? s.headAge : 'n/a')
                      + ' cadence=600');
                  } catch (_) {}
                };
                try {
                  const s = window.__STABLES_ANCHOR__ || {};
                  /* Two different "no", and they are not the same thing to a person reading this:
                     the switch is a choice, an unpublishable release is a property of the build. */
                  ...
                  Promise.resolve(tv81AnchorPublishSnapshot()).then(function (r) {
                    r = r || {};
                    if (r.published) say('wrote', 'records=' + r.records, s);
                    else if (r.skipped) say('refused', String(r.skipped), s);
                    else if (r.error) say('failed', String(r.error), s);
                    else say('refused', 'no-result', s);
                  }).catch(function (e) { say('failed', String((e && e.message) || e), s); });
                } catch (e) { say('failed', 'check-threw:' + ((e && e.message) || e), null); }
              };
              ...
              tv81AnchorGapFill().then(function () {
                ...
                anchorPublishIfStale('boot');
              }).catch(function (e) {
                try { console.log('[STABLES-ANCHOR] pass error: ' + ((e && e.message) || e)); } catch (_) {}
              });
              /* Periodic re-pass, well inside the 600-block cadence and cheap when nothing changed.
                 LEFT AT THIRTY MINUTES ON PURPOSE. The instinct on finding a job that never fires is
                 to make it fire more often, and here that would be wrong twice over. The staleness
                 threshold is 600 blocks, about eight hours, so the snapshot essentially cannot go
                 from fresh to stale inside one foreground session: the repeat almost never has
                 anything new to decide, and the boot check above already covers the session. Raising
                 the rate would triple a background pass against the battery law for a case that does
                 not occur. The defect was never the interval, it was that the boot pass did not
                 check at all. */
              window.stablesRepeatWhileVisible('anchor-gapfill', function () {
                tv81AnchorGapFill().then(function () {
                  try { tv81AnchorStatusLine(); } catch (_) {}
                  anchorPublishIfStale('repeat');
                }).catch(function () {});
              }, 1800000);
            }, 60000);
          }).catch(function (e) {
