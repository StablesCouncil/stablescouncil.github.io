...
- **Release metadata aligned for v0.0.11.88.** Packages the approved notes grouping and earlier startup and reliability changes together.

- **Notes are grouped by address.** The currency notes sheet shows an address heading above each group, retaining each note's amount, age and selection. Select all still covers every address, and grouping uses the existing node response.

### Release preparation

### Fixed
- **Coordinated public test release, v0.0.11.86 (2026-09-10).** Packages the complete changes since the verified public .63 release across Web, MiniDapp and both Android apps. Cold-payment timing and isolated battery performance remain under measurement.
- **Concurrent Android startup, v0.0.11.85 (2026-09-10).** Load the app while the embedded node starts. Native commands still require node readiness and payment actions still require fresh wallet proof. Refresh when boot completes, preserve resync completion checks, and stop startup observation while backgrounded. Device timing validation is recorded in the retail infrastructure plan.
- **Shared receive-address reads, v0.0.11.84 (2026-09-10).** Concurrent receive-address callers share one node read. Connection and wallet changes invalidate pending answers and cached addresses. Slow MDS replies no longer launch a duplicate RPC fallback. Added bounded local payment-stage timing without storing payment details. Retail performance remains under measurement.
- **Retail payment reliability, v0.0.11.83 (2026-09-10).** Failed wallet proofs remain retryable with bounded ambient retries instead of being marked fresh. Merchant QR payments carry a unique invoice identity, exact recipient, token, amount and expiry. Detection no longer waits for wallet balance reconstruction. Goods release checks the matching transaction against the merchant's selected confirmation threshold, including a fresh check when releasing. Unknown confirmation keeps release disabled. Active invoices resume locally and released receipts retain their transaction identity. Corrected history-test ownership expectations and added retail regression checks. Development validation and remaining physical checkout and battery measurements are recorded in RETAIL_PAYMENT_INFRASTRUCTURE_PLAN.md; no new APK release or speed guarantee.

<!-- Builds .64-.82 were prepared locally but not publicly released. Included below. -->

### Added
- **The xWiniwa vault runs as four independent lanes, live on mainnet.** Until now the vault was one
  set of coins, so a second mint or burn had to wait for the first to be mined. One address now holds
  four complete copies of the vault, told apart by a lane number the coins carry, so a mint on one
  lane and a burn on another never touch the same coin and can be mined together. A mint goes to the
  lowest free lane with reserve, a burn to the lowest free lane whose pool covers it, and the hold
  that used to lock the whole vault now locks one lane, so one person can mint and burn in the same
  block. A burn no single lane can cover is refused in words, naming the largest lane's amount. The
  original vault is not retired: it keeps its Winiwa pool, takes any burn no single lane covers so it
  drains as testers use it, and still answers for anyone on an older build. Amounts, wording and the
  confirmation step are unchanged. The lane vault
  `0x0C402031CE74B8C4F65CDC19F9E12AC00526091920C76E72AC90FAD7C57D5606` was seeded from Cold through
  Hot with 1,000,000 xWiniwa per lane. Design: `task_test_channel/XWINIWA_VAULT_V2_SAME_BLOCK_DESIGN.md`.

### Changed
- **Each currency now says what is still arriving.** A faucet claim or a mint lands in the balance
  only once the network has confirmed it, so for a few minutes the figure on a currency row was a
  number you could not act on: you could not tell whether the thousand Winiwa you had just claimed
  was already in it or still to come, and the row in Recent activity underneath did not settle the
  question either, because nothing said whether that row had been counted yet. The row now carries the amount on
  its own, "+1,000.00", directly under the balance, and it appears only while that amount is
  genuinely not in the figure above it, so a plus figure always means "on top of what you see" and
  no plus figure means everything is counted. It goes away by itself once the amount is final. A
  claim or a mint, which the app credits to your balance the moment it is sent, adds no figure
  because there is nothing to add; the row still pulses while it confirms. The pinned currency
  at the top of the wallet, which is where most people look first, had no settling signal of any
  kind before this and now carries the same line and the same amber pulse as the rows below it.
- **One rate line, and tapping it turns it around, on Mint, Burn and Exchange alike.** Tap the rate
  and it reads the other way round. Mint and Burn already did this; Exchange did not. It put sentences
  where the rate belongs, "Reading the order book", "No offers on this side yet", "Best so many Winiwa
  per xWiniwa", and when it did show a rate it ignored the tap altogether. Those sentences now sit on
  the line underneath, which already existed for exactly that and stays out of the way when there is
  nothing to say, and the rate reads the same on all three screens.

- **The Mint page prices xWiniwa in Winiwa, shows the leverage, and says each number once.** The price
  line asked an outside exchange for a dollar figure the app then refused to show, because these are
  valueless test tokens, so it sat empty for ever. It now says what one xWiniwa costs in Winiwa, which
  is the only price the system actually has, and for now that is one. The leverage line was empty too,
  and it did not need to be: with no stablecoin in existence there is no senior debt, so the leverage
  is exactly one, and that is arithmetic rather than a guess. The amount you would receive was printed
  twice, once under the amount you type and again just above the button, so the second one is gone and
  the one that stays carries the exact figure. The button says "Confirm mint" instead of repeating the
  amount a third time, which also stops a long number wrapping the label.

- **Your app now helps someone who has just installed Stables, unless you turn that off.** A node only
  sees coins its own window still reaches, so a brand new install after a quiet day cannot find the
  faucet or the vault and cannot claim or mint. Your app writes a small proof to Minima that any node
  reads for itself, with no server in the way. One proof serves everybody: an app only writes one when
  nobody has written one in about eight hours, so it happens a few times a day across all of us rather
  than once per person, and it costs a fraction of a Winiwa. The switch sits in Settings under
  "Helping other people" and says what it does. It used to read "Publish book snapshots" under a
  heading called Order book, in a release that has no order book, so it told a reader nothing and
  could not be switched on at all.

### Fixed
- **MAX offers only what you can actually spend, so a mint straight after a claim no longer fails.**
  Claiming 1,000 Winiwa and then pressing MAX on the Mint page put 6,468.92507698 in the field
  against a balance of 5,468.93, and the transaction was refused with "Not enough Winiwa in your
  wallet". A claim is credited to your balance the moment it lands, which is right, because the
  money is yours; but coins cannot be spent until the network has confirmed them a few blocks
  later, and that credit was being written into the spendable figure as well as the owned one. MAX
  now reads what can be moved, so it can never offer more than the chain will accept, while the
  balance you read still shows the claim straight away. A payment on its way out still lowers both
  figures at once, which is what that same mechanism was added for.
- **Hiding your amounts now really hides them while money is arriving.** With the eye closed a
  currency still glowing and pulsing gave away that something was landing, and roughly how much,
  because the rule written to stop that named a class the app has never used. The blur was doing
  its job and the animation underneath was not covered by it.
- **Opening the app no longer stalls while it does everything at once.** Coming back to Stables with
  your node already running still took several seconds before the screen settled, and none of that
  wait was the node: measured on the phone, the app allocated 462 MB and stopped three times for
  garbage collection while it did it. Returning to the app fired all twenty-seven of its repeating
  jobs in the same instant, so the work of a whole minute arrived in one breath and the screen could
  not paint until it cleared. Those jobs are now spread across about a second and a half. None is
  skipped and the order has not changed, and the figures you actually look at do not wait for them:
  your balance and the latest block are asked for first, before any of it.

- **"See all" now sits on the same line as "Recent activity" instead of floating above it.** The row
  was aligning the boxes rather than the words, and the action is a large tap target with a small
  label centred inside, which left the label sixteen pixels high. The label moved; the tap target is
  the same size as before.

- **A mint and a burn started together no longer collide, which is what the lanes are for.** Two
  faults, one behind the other. The first was a guard: starting a burn while a mint was still being
  prepared answered "A mint or burn is already in progress", on a build whose whole point is that
  those two no longer collide. A mint spends Winiwa and a burn spends xWiniwa, so they take different
  coins from your wallet, and the vault sends them to different lanes, so they take different coins
  from the vault. The lane work taught the vault hold about lanes but missed this second, older
  guard, a single switch for both directions that refused everything while either was building.
  Lifting it exposed the second fault underneath, found by running the two together for real: both
  operations chose the same lane and then destroyed each other's half-built transaction. A lane was
  only marked busy after its transaction reached the chain, so for the whole minute or two of
  building it still looked free, and the draft was named after the lane alone, so the second
  operation deleted the first one's draft before making its own. One settled and the other died
  saying it could not find a transaction. A lane is now busy from the moment it is chosen, and a
  draft carries its direction, so nothing of one operation can be mistaken for another's. Two mints
  still wait for each other, because they really would reach for the same Winiwa coins, and the
  message now says which direction is busy instead of naming both.

- **Exchange says when it opens, instead of quoting a book this test does not have.** Asking to
  exchange five Winiwa answered "The book only covers 0.4425 Winiwa of this size", which is a real
  measurement of an order book that this release does not include, offered to someone who cannot act
  on it either way. The page reads the release's own list of what is switched on before it quotes
  anything, and says plainly that exchanging opens when the stablecoins are minted, and that until
  then you can mint and burn xWiniwa on the Mint page. The route stays reachable and the screen stays
  readable, which is the standing rule for anything this release defers.

- **A button you cannot press no longer looks like the one you should press.** On Exchange, asking for
  more than the order book could cover left the button in full colour, inviting a tap the app was
  always going to refuse with a message. It is now shown in the muted, unavailable colour and cannot
  be pressed, and the line beside it still says what to do instead. This was not only an Exchange
  problem. The rule that paints a money-moving button had no notion of an unavailable state at all and
  overrode anything the page tried to say, so **every disabled transaction button in the app looked
  available**. The rule itself now carries that state, so the fix holds wherever it applies rather
  than on one screen.

- **A currency this test does not have says so, everywhere it is offered.** In the currency lists,
  USDw, EURw, GBPw and the rest showed a balance of 0. That reads as "you hold none of this", which is
  a different and misleading thing to say about a token that does not exist in this release at all.
  They now say **Unavailable**. Three separate faults sat behind one symptom. Three pieces of code
  wrote that line and the one that fills in balances ran last, painting 0 over the top, so there is
  now one place that decides what the line says. The same list said "Unavailable" on one screen,
  "Later" on another and "Soon" on a third for the same question, so all three now take one word from
  one place. And the invoice currency list matched none of the rules that mark a token unavailable, so
  it went on offering eight stablecoins that do not exist; every currency list now asks the release's
  own asset boundary, which is the single place that already knew the answer.

- **An error told you to switch on something you had already switched on.** Minting a large amount
  could fail with a message asking you to turn on combining notes in Wallet management, which the
  founder read with that setting already on. Two things were wrong behind one sentence. Mint and burn
  already combine notes by themselves when they need to, and in this case the notes did fit, so
  combining was never the missing step. And one of the two errors behind that message is about a reply
  being too large for the node to return, not about a transaction being too large, so it does not mean
  too many notes at all. The message now tells the two apart and never asks for a setting that is
  already on.

- **Your app was never actually writing the proof that helps a brand new install, even with the switch
  turned on, and now says whether it wrote and why not.** The faucet and vault coins are only visible
  to a node whose unpruned window still reaches them, roughly seventeen hours on a quiet channel. The
  answer has always been a snapshot published on chain carrying those coins' proofs, which any node
  imports and can then claim and mint from with no hosted service. Measured on 2026-09-07: no snapshot
  for this generation existed on chain at all. Three faults had to be cleared. The decision to write
  was still asking whether this release has an order book, which it does not, so it answered no every
  time; it now asks what the snapshot can actually carry, so a release with the faucet and the vault
  publishes a faucet-and-vault snapshot. The check that makes that decision ran only inside a job
  needing the app left open on screen for a continuous half hour, so ordinary use never reached it; it
  now also runs the moment the app finishes its first look at the chain. And the whole fault survived a
  full day looking perfectly healthy, because an app that is working writes nothing to the screen and a
  broken one writes nothing either, so each decision is now recorded in one line: whether it wrote, or
  held back and for which reason, together with the figures it decided from, and the result when it
  does write. Proven by driving the real app against a live node: the check fires, and a fresh proof
  was written to Minima. Publishing stays off unless it is switched on.

- **A resync that fails says why, the default recovery node has a fallback, and a build without a node
  of its own no longer blames the address.** Founder report (2026-09-06, on the web preview): "the
  default address didnt work". The address was fine: a real `megammrsync` from a fresh desktop node
  against `spartacusrex.com:9001` completed in 13 seconds, and against the official
  `megammr.minima.global:9001` in 15. What failed was the message. The web build and the MiniDapp have
  no node of their own to repair, and the Minima Core companion's node belongs to Core, yet pressing
  Resync there produced the same sentence as a real failure, and on the Android app every failure
  produced that one sentence too, whatever had happened. Now: on a build without a node of its own the
  sheet says so the moment it appears and the button stays off; on the Android app the native side
  reports a reason in a closed vocabulary (unreachable, no MegaMMR data, a node error with the node's
  own words, not finished, interrupted, stopped unexpectedly) and the sheet turns it into one sentence
  naming the address; and when a known address fails before anything was replaced, the app tries the
  next known MegaMMR node on its own and says so. An address the person typed is never swapped.
